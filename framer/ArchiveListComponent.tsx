...
/**
 * Framer draws this component inside frames it sizes itself, and on the
 * published /archive page those frames carry hard pixel heights with
 * `overflow: clip` — measured 2026-08-27: a 720px section wrapping 1241px of
 * journal. The component sized itself correctly and the page still could not
 * scroll, because nothing ever grew past the viewport for the browser to
 * scroll. Cards below the fold were clipped, not hidden below a scrollbar.
 *
 * `@framerSupportedLayoutHeight auto` does not help: it governs this
 * component's own frame, not the frames above it.
 *
 * The real fix is one setting in the Framer editor — set that section's
 * height to Fit Content. This exists because the page should not silently
 * swallow two thirds of the journal while that goes unnoticed. It only ever
 * relaxes a constraint: a fixed height becomes auto, a clip becomes visible,
 * an absolute container becomes relative so it contributes its height to the
 * flow again. An ancestor that is already auto-sized is left untouched, so
 * this is a no-op once the frames are set correctly.
 */
function unclipFramerAncestors(node: HTMLElement | null): void {
  let element = node?.parentElement ?? null;
  while (element && element !== document.body) {
    const computed = window.getComputedStyle(element);
    // Absolute children contribute nothing to a parent's height, which is why
    // the 1080px frame above measured 0px tall.
    if (computed.position === "absolute") element.style.position = "relative";
    if (computed.overflowY === "clip" || computed.overflowY === "hidden") {
      element.style.overflow = "visible";
    }
    if (computed.height !== "auto") element.style.height = "auto";
...
export default function ArchiveListComponent() {
  ...
  // Re-run on every size change, not once on mount: the journal is empty on
  // the first paint and grows when the fetch resolves, and Framer re-applies
  // its own inline heights on resize. A ResizeObserver covers both, plus the
  // reflow when a lazy thumbnail finally loads.
  useEffect(() => {
    const root = rootRef.current;
    if (!root || typeof ResizeObserver === "undefined") {
      unclipFramerAncestors(root);
      return;
    }
    const observer = new ResizeObserver(() => unclipFramerAncestors(root));
...
const styles: Record<string, CSSProperties> = {
  ...
  page: {
    ...
    width: "100%",
    margin: 0,
    // 96px, not 40: Framer anchors the page footer to the bottom of the
    // section this component sizes, so once the section auto-sizes the
    // footer lands inside that padding. At 40px it overlapped the last row
    // of cards by 7px. Measured on the published page, 2026-08-27.
