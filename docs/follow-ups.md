...
The obvious fix is a small per-day JSON file generated by the GitHub Action
that already builds the archive. It needs a design decision first, because
the current client-side date arithmetic is *robust* — it computes the right
animal even if the daily job fails, is late, or never ran. A pre-generated
per-day file gives that up: a failed job means a stale or missing animal for
everyone. Options worth weighing are a per-day file with the client-side
computation kept as a fallback, or a chunked list, or accepting the payload
until the animal count actually justifies the change.
