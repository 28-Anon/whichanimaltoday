...
### Open: the archive renders every entry eagerly

`ArchiveListComponent` maps over all entries with no pagination and no
`loading="lazy"` on the thumbnails. At 34 entries this is fine; at 500 it is
500 image requests on one page load. Performance rather than security, and the
same growth curve as the payload issue above. Cheapest fix by far is
`loading="lazy"` on the `<img>`; pagination or windowing only if that proves
insufficient.

### Open, minor: `entry.slug` is interpolated into a URL unencoded

`ArchiveListComponent` builds `href={`/archive-detail?slug=${entry.slug}`}`
without `encodeURIComponent`. Not an injection route — the value lands in a
relative path that always begins `/archive-detail?`, so it cannot become a
`javascript:` URL — but a slug containing `&`, `#`, or a space would break the
query parameter. Slugs are generated by `scripts/slug.ts` from curated animal
names so this is currently unreachable; `encodeURIComponent` is still the
correct call.

### Noted: `imageUrl` is not validated

Animal and archive image URLs go straight into `<img src>` with no scheme
check. The mitigation is provenance: they come from `data/animals.json` in
this repo, which only the owner can write to, and `<img src="javascript:…">`
does not execute in any current browser. Worth an `https://` assertion in the
CSV import step if third-party contributions ever become possible.
